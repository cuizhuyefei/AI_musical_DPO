...
def eval_model(tokenizer, policy_model, data):
    ...
    for idx, prompt in enumerate(batch['prompt']):
        print("prompt:", prompt)
        print("sample:", [reference_samples[i][len(prompt)+1:] for i in range(5*idx, 5*idx+5)])
        desired_length = [int(n) for n in re.findall(r'[-+]?[0-9]*\.?[0-9]+', prompt)][0]
        desired_gen = []
        ...
        content = {'prompt': prompt, 'desired_gen': desired_gen}
        res.append(content)

        print("\n")
    # for prompt, sample in zip(inputs, reference_samples):
    #     print("prompt:", prompt)
    #     print("sample:", sample)
    #     print("\n")
    print("------------------------------------------------------------------------")
