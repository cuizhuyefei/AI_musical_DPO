        ...
        for k in batch[0].keys():
            if k.endswith('_input_ids') or k.endswith('_attention_mask') or k.endswith('_labels'):
                if 'prompt' in k:  # adapted from https://stackoverflow.com/questions/73256206
                    to_pad = [torch.LongTensor(ex[k][::-1]).to('cuda:1') for ex in batch]
                else:
                    to_pad = [torch.LongTensor(ex[k]).to('cuda:1') for ex in batch]
...
def gen_test(tokenizer, policy_model, num_data):
    ...
    raw_data = get_dataset(name='prompt_refine', split='test')
...
def test_for_ckpt(tokenizer, policy_model, ckpt_path = None, save_path = None):
    if ckpt_path:
    ...
    with open(save_path, 'w', encoding='utf-8') as file_obj:
        json.dump(gen_res, file_obj, indent=4, ensure_ascii=False)
...
if __name__ == '__main__':
    ...
    test_for_ckpt(tokenizer, policy_model, '.cache/zhuorui/prompt_refine_2024-01-15_19-43-45_463291/LATEST/policy.pt', 'prompt_refine_test.json')
